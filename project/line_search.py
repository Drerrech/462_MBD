...
def quadratic_interpolation_line_search(f, x_k, d_k, g_approx_k, armijo_eta, max_search_it=8, max_tune_it=1, f_post_process=lambda y: y, delta=1):
    f_at_x_k = f_post_process(f(x_k))
    ...
    for i in range(max_tune_it):
        l_star = ((l2**2 - l3**2)*f1 + (l3**2 - l1**2)*f2 + (l1**2 - l2**2)*f3) / (2*((l2 - l3)*f1 + (l3 - l1)*f2 + (l1 - l2)*f3))
