...
def mbd_basic(f, x, grad_approx, line_search, log_path, delta=1, target_acc=1, armijo_n=0.05, eps_d=0.1, eps_stop=1e-4, max_f_evals=1e16, f_post_process=lambda y: y):
    ...
    def log_progress(msg=""):
        s = f"{k:4} | {str(x.tolist()):80} | {f_post_process(f_val_at_x):32.2f} | {delta:6.4f} | {target_acc:10.4f} | {norm_g_approx:6.2f} | {f_evals:7} | {success:7} | {msg}"
    ...
    # 3) line-search
            ...
            # perform line search
            t = line_search(p_reuse.evaluate, x, d, g_approx, armijo_n, f_post_process=f_post_process)
